...
# 1. Write code that will help us to get cars that satisfy search_criteria.
cars_filtered = []
for key, val in car_data.items():
    if (val[1] >= search_criteria[0]
            and val[2] >= search_criteria[1]
            and val[4] <= search_criteria[2]):
        cars_filtered.append((key, val))
# 2. Cars should be sorted by price ascending.
sorted_cars = sorted(cars_filtered, key=lambda x: x[1][-1])
# 3. We should print up to five first found elements.
sliced_cars = sorted_cars[:5]
print(sliced_cars)
